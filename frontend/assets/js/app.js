...
if (product.created_at) {
  ...
  isNew = ageInDays >= 0 && ageInDays <= 5;
}

if (quantity <= 0) {
  return {
    badge: "SOLD OUT",
    badgeClass: "sold-out-badge",
    canAddToCart: false,
  };
}
              ...
              <div class="product-card-inner">
                  <div class="product-card-front">
                      <div class="product-image">
                          ...
                          ${
                            stockState.badge
                              ? `<span class="product-stock-badge ${stockState.badgeClass}">${stockState.badge}</span>`
                              : ""
                          }
                      ...
                      <div class="product-info">
                          ...
                          <button
                            class="add-to-cart-btn ${
                              !stockState.canAddToCart ? "out-of-stock-btn" : ""
                            }"
                            onclick="addToCart(event, '${product._id}')"
                            ${!stockState.canAddToCart ? "disabled" : ""}
                          >${
                            stockState.canAddToCart ? "Add to Cart" : "Sold Out"
                          }</button>
                      </div>
                  </div>
...
try {
  ...
  const availableQuantity = Number(productToAdd.quantity || 0);

  if (availableQuantity <= 0) {
    showToast("This piece is currently sold out.", "error");
    return;
  }
  ...
  if (existingItem) {
    if (existingItem.quantity >= availableQuantity) {
      showToast(`Only ${availableQuantity} available.`, "error");
      return;
    }
