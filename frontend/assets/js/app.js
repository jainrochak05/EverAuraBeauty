...
async function loadAllProducts(category = 'all') {
    ...
    productsToDisplay.forEach(product => {
        ...
        let slides = product.images && product.images.length > 0 ? product.images.map(imgUrl => `<div class="swiper-slide"><img src="${imgUrl}" alt="${product.name}"></div>`).join('') : `<div class="swiper-slide"><img src="https://via.placeholder.com/400x550?text=No+Image" alt="${product.name}"></div>`;
...
async function loadTestimonials() {
    ...
    try {
        ...
        new Swiper('.testimonial-swiper', { loop: testimonials.length > 1, autoplay: { delay: 5000, disableOnInteraction: false }, pagination: { el: '.swiper-pagination', clickable: true }, });
...
async function loadAllReviewsInteractive() {
    ...
    try {
        ...
        container.innerHTML = '';
        if (testimonials.length === 0) {
            container.innerHTML = '<p style="text-align: center;">No reviews have been posted yet.</p>';
            return;
        }
...
function openReviewModal(reviewData) {
    ...
    modal.innerHTML = `
        <div class="review-content">
            ...
            ${reviewData.full_review ? `<p class="full-review">${reviewData.full_review}</p>` : ''}
...
async function handleTestimonialSubmit(event) {
    ...
    const data = { name: document.getElementById('testimonial-name').value, contact: document.getElementById('testimonial-contact').value, summary: document.getElementById('testimonial-summary').value, full_review: document.getElementById('testimonial-full').value };
    try {
        const response = await fetch(`${API_URL}/testimonials`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(data) });
        if (!response.ok) throw new Error('Submission failed.');
        closeModal('testimonial-modal');
        showToast('Thank you! Your review is awaiting approval.');
        document.getElementById('testimonial-form').reset();
    } catch (error) { console.error('Error submitting testimonial:', error); alert('There was an error submitting your review.'); }
}
...
function openCartModal() {
            ...
            itemsHTML += `
                <li class="cart-item">
                    ...
                    <div class="cart-item-details">
                        ...
                        <span class="cart-item-price">Qty: ${item.quantity} × ₹${item.price.toFixed(2)}</span>
                    </div>
    ...
    const storedCoupon = JSON.parse(sessionStorage.getItem("appliedCoupon") || "null");
    let total = subtotal;
    ...
    cartTotals.innerHTML = `
        ...
        <p>Total after coupon: <span id="cart-total">₹${total.toFixed(2)}</span></p>
    `;
...
function updateCouponUI(coupon) {
    ...
    if (coupon) {
        ...
        document.getElementById("applied-coupon-code").textContent = `${coupon.code} - ${coupon.discount}% OFF`;
        couponInput.value = "";
    } else {
